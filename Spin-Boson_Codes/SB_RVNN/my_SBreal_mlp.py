...
class SpinBosonRealPINN(nn.Module):
    def __init__(self, sequence_length, state_size, hidden_size, ostl_steps):
        ...
        input_size = sequence_length * state_size  # e.g., 324
        output_size = ostl_steps * state_size  # e.g., 160
...
### Trace Penalty Loss Function (rho_11 + rho_22 ≈ 1 for each time step)
def trace_penalty(pred):
    # pred shape: (batch_size, pred_steps, 4)
    # Indices: 0 for rho_11, 3 for rho_22 (both real)
    rho_11 = pred[:, :, 0]  # Shape: (batch_size, pred_steps)
    rho_22 = pred[:, :, 3]  # Shape: (batch_size, pred_steps)
    # Trace condition: rho_11 + rho_22 = 1 for each time step
    trace = rho_11 + rho_22  # Shape: (batch_size, pred_steps)
...
def train_model(model, train_X, train_Y, val_X, val_Y, epochs, lr, lambda_trace, save_dir="saved_models"):
    ...
    print(f"Losses saved to 'losses.npz'")
...
if __name__ == "__main__":
    ...
    model_real = SpinBosonRealPINN(sequence_length=81, state_size=4, hidden_size=64, ostl_steps=ostl_steps).to(device)

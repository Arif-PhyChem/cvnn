...
sys.path.append("D:/4_site_Hyp.py")
from FMO4SiteRV_mlp import SpinBosonRealPINN, RealLinear
...
model = torch.load(
    r"D:\4_site_Hyp_RVNN\saved_models_4siteRV_HYP\real_mlp_model-49879-tloss-6.9817e-07-vloss-8.8735e-07.pth",
    map_location=device,
    weights_only=False
)
model.eval()
...
# Pick ONE test file
test_file = r"D:\4_sites_hypothetical\4_sites_hypothetical\test_data\4_initial-1_gamma-375.0_lambda-100.0_temp-150.0.npy"
print("Running predictions on:", test_file)

# Directory to save results
save_dir = "D:/4_site_Hyp.py/results"
...
ax1.set_title("4-Site Hyp Model: RVNN", fontsize=45)
ax1.tick_params(labelbottom=False)
ax1.grid(True)
ax1.legend(fontsize=25, ncol=1, loc='upper right')

# Off-diagonals
ax2 = axs[1]
off_diag_indices = {"ρ₁₂": (1, 2), "ρ₂₃": (8, 9), "ρ₃₄": (13, 14)}
...
for label, (re_idx, im_idx) in off_diag_indices.items():
    ...
    ax2.plot(np.arange(81, 401) * time_step_ps, predicted_trajectory[:, re_idx], '-', linewidth=4.5, color=re_colors[label],
             marker='o', markevery=10, ms=10, alpha=0.9)
...
ax2.grid(True)
ax2.legend(fontsize=25, ncol=1, loc='lower right')

...
ostl_steps = 80
sequence_length = 81
state_size = 10  # Adjusted for 4_site hypothetical model (complex) (4 sites)
...
def trace_penalty(pred):
    """
    Compute trace penalty for pred tensor of shape (batch_size, num_matrices, 10),
    where 10 elements correspond to upper-triangular entries (including diagonal)
    of a 4x4 matrix in row-major order.
...
#saved_models = []  # Each entry is (loss_value, filepath)
def train_model(model, train_X, train_Y, val_X, val_Y, epochs, lambda_trace, lr, save_dir="saved_models"):
    ...
    for epoch in range(epochs):
        ...
        # Save model and keep only best models
    ...
    np.savez('4site_cvnn_losses.npz',
    ...
    print("Loss history saved to '4site_cvnn_losses.npz'")
...
if __name__ == "__main__":
    ...
    model = SpinBosonCVPINN(sequence_length=sequence_length, state_size=state_size, hidden_size=64, ostl_steps=ostl_steps)

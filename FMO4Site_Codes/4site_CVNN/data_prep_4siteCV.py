...
data_dir = "D:/4_sites_hypothetical/4_sites_hypothetical/training_data/*.npy"  # Adjust this to your actual directory
...
def process_trajectory(file_path):
    ...
    #print(len(X_list))
    X = np.array(X_list)  # Shape: (4, 81, 10)
    Y = np.array(Y_list)  # Shape: (4, 80, 10)
    return X, Y
...
all_X = np.concatenate(all_X, axis=0)  # Shape: (num_files * 4, 81, 10),
all_Y = np.concatenate(all_Y, axis=0)  # Shape:  (num_files * 4, 80, 10)

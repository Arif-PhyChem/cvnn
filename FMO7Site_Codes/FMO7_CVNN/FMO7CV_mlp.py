...
def trace_penalty(pred):
    """
    Compute trace penalty for pred tensor of shape (batch_size, num_matrices, 28),
    where 28 elements correspond to upper-triangular entries (including diagonal)
    of a 7x7 matrix in row-major order.
    ...
    diag_indices = [0, 7, 13, 18, 22, 25, 27]
...
def train_model(model, train_X, train_Y, val_X, val_Y, epochs, lambda_trace, lr, save_dir="saved_models"):
    ...
    print("Loss history saved to 'fmo_cvnn_losses.npz'")
...
if __name__ == "__main__":
    ...
    model = SpinBosonCVPINN(sequence_length=sequence_length, state_size=state_size, hidden_size=108, ostl_steps=ostl_steps)

...
def process_trajectory(file_path):
    ...
    #print(len(X_list))
    X = np.array(X_list)  # Shape: (4, 81, 28)
    Y = np.array(Y_list)  # Shape: (4, 80, 28)
...
all_X = np.concatenate(all_X, axis=0)  # Shape: (num_files * 4, 81, 28),
all_Y = np.concatenate(all_Y, axis=0)  # Shape:  (num_files * 4, 80, 28)

...
data_dir = "E:/7_sites_adolph_renger_H/training_data/init_1/*.npy"  # Adjust this to your actual directory
...
def process_trajectory(file_path):
    traj = np.load(file_path)  # Shape: (10001, 49), complex
    print(traj.shape)
    states = traj[:401, 1:]  # Extract rho elements
    # Prepare real-valued features
    real_states = np.zeros((401, n_states ** 2))  # Shape: (401, 49)
    q = 0
    for p in labels:
        if p % divider == 0:
    ...
    X = np.array(X_list)  # Shape: (320, 81, 49)
    Y = np.array(Y_list)  # Shape: (320, 49)
    return X, Y
...
all_X = np.concatenate(all_X, axis=0)  # Shape: (num_trajectories * 320, 81, 49)
all_Y = np.concatenate(all_Y, axis=0)  # Shape: (num_trajectories * 320, 80, 49)

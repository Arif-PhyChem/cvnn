...
def process_trajectory(file_path):
    ...
    real_states = np.zeros((401, n_states ** 2))  # Shape: (401, 64)
    q = 0
    for p in labels:
        if p % divider == 0:
    ...
    X = np.array(X_list)  # Shape: (4, 81, 64)
    Y = np.array(Y_list)  # Shape: (4, 80, 64)
...
all_X = np.concatenate(all_X, axis=0)  # Shape: (1600, 81, 64)
all_Y = np.concatenate(all_Y, axis=0)  # Shape: (1600, 80, 64)
